# Reservoir Optimization Toolkit

---

# Part I — `res_tool_lp.py` Method Documentation
...
# Part II — `rtc_tool_app` Directory Guide
...
| File | Description |
|---|---|
| `timeseries_import.csv` | Input inflow from WVIC data at 6-hour step |
| `Reservoir.mo` | Modelica file specifying the parameters and variables |
| `model_single_res.py` | Python script using RTC-Tools to read the input file and solve the single model using LP solver |
| `timeseries_export.csv` | Output optimal release quantity and inflow at 6-hour step |
...
|---|---|
| `timeseries_import.csv` | Input inflow from WVIC data at 6-hour step |
| `MultiReservoirSystem.mo` | Modelica file specifying the parameters and variables |
| `model_multi_res.py` | Python script using RTC-Tools to read the input file and solve the multi model using LP solver |
| `timeseries_export.csv` | Output optimal release quantity and inflow at 6-hour step |

---

## How to Use (Steps 1-3 are optional. if you already has rtc_tool installed)
...
4. **Execute the script:**
   ...
   Navigate to the Python script path and run:
   ```bash
   cd .rtc_tools_setup/src
   python optimization_problem.py
   ```
   The optimized quantity will be saved in the `output/` folder.

5. ** Do not change the folder structure** — the model will report errors if the directory layout is modified.
